	...
	public UpdateCheck() throws SQLException {
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) { 
				...
				String s2 = txt_ID.getText(); //room_number;    
                                ...
                                conn.s.executeUpdate("update customer set room_number = '"+s2+"', name = '"+s3+"', status = '"+s4+"', deposit = '"+s5+"' where number = '"+s1+"'");
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String s1 = c1.getSelectedItem();
				Conn conn = new Conn();
                                ResultSet rs1 = conn.s.executeQuery("select * from customer where number = "+s1);
                            ...
                            try{
                                String total = "";
                                Conn conn  = new Conn();
                                ResultSet rs2 = conn.s.executeQuery("select * from room where roomnumber = '"+txt_ID.getText());
                                while(rs2.next()){
